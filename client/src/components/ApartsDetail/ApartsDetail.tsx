import React from "react";

export default function ApartsDetail() {
  return (
    <div className="feedback">
      <h2 className="h2-related" >CHI TIẾT</h2>
      <p style={{ textAlign: "justify" }}>
        <span style={{ fontSize: "16px" }}>
          Cú Trên Cây Homestay có vị trí địa lý và đặc điểm đặc biệt so với các
          nơi ở khác nên bạn cần chú ý một vài lưu ý dưới đây:
          <br></br>
        </span>
      </p>
      <ul style={{ textAlign: "justify" }}>
        <li>
          <span style={{ fontSize: "16px" }}>
            Home cách trung tâm Đà Lạt khoảng 2km, sân bay Liên Khương khoảng
            28km. Tuy nhiên home ở lưng chừng đồi, có dốc và nhiều bậc thang
            nhưng an toàn. NẾU NGẠI DI CHUYỂN VÀ CÓ NHIỀU HÀNH LÝ NẶNG, bạn hãy
            cân nhắc thật kỹ.
          </span>
        </li>
        <li>
          <span style={{ fontSize: "16px" }}>
            Trả tổ trước 12h, nhận tổ sau 14h. Luôn trở về tổ trước 23h để đảm
            bảo an toàn và giữ gìn sự yên tĩnh cho cả làng chim.
          </span>
        </li>
        <li>
          <span style={{ fontSize: "16px" }}>
            Hạn chế âm lượng khi dùng loa nghe nhạc và sinh hoạt cá nhân (đi
            lại, nói chuyện to tiếng..)
          </span>
        </li>
        <li>
          <span style={{ fontSize: "16px" }}>
            Home trồng nhiều cây xanh và nuôi nhiều chó mèo.
          </span>
        </li>
      </ul>
      <p>
        <span style={{ fontSize: "16px" }}>
          Đến với Cú Trên Cây Homestay Cú khách có nhiều không gian sử dụng khác
          bao gồm:
        </span>
      </p>
      <p>&nbsp;</p>
      <p style={{ textAlign: "center" }}>
        <img
          style={{ height: "229px", width: "344px" }}
          src="https://cutrencay.com/wp-content/uploads/2020/08/WC-1-e1632355477814-600x400.jpg"
          alt=""
        />
        <img
          style={{ height: "229px", width: "344px" }}
          src="https://cutrencay.com/wp-content/uploads/2020/08/z2786458624933_afb35256ed3a15d797b035f671e6cdb3-scaled-e1632407230712-600x400.jpg"
          alt=""
        />
      </p>
      <p style={{ textAlign: "justify" }}>
        Toilet chung có tận 4 phòng, chỉ cách tổ vài bước chân và được kiểm tra
        vệ sinh thường xuyên nên rất sạch sẽ và khô ráo. Được t
        <span style={{ fontSize: "16px" }}>
          rang bị đầy đủ máy nước nóng, khăn tắm, máy sấy tóc, dầu gội, xà
          phòng, kem đánh răng, kem chống muỗi, Khách lưu trú chỉ cần đem theo
          bàn chải đánh răng.
        </span>
      </p>
      <p>&nbsp;</p>
      <p style={{ textAlign: "center" }}>
        <img
          style={{ height: "229px", width: "344px" }}
          src="https://cutrencay.com/wp-content/uploads/2020/08/shccutrencay-e1632355358503-600x400.jpg"
          alt=""
        />
        <img
          style={{ height: "229px", width: "344px" }}
          src="https://cutrencay.com/wp-content/uploads/2020/08/caucheckin-e1632488390370-600x400.jpg"
          alt=""
        />
        <img
          style={{ height: "229px", width: "344px" }}
          src="https://cutrencay.com/wp-content/uploads/2020/08/SHC2-1-600x400.jpg"
          alt=""
        />
      </p>
      <p style={{ textAlign: "justify" }}>
        <span style={{ fontSize: "16px" }}>
          Cú Trên Cây Homestay có nhiều khu sinh hoạt để Cú khách nghỉ ngơi, làm
          việc, đọc sách, hòa mình vào thiên nhiên.
        </span>
      </p>
      <p>&nbsp;</p>
      <p style={{ textAlign: "center" }}>
        <img
          style={{ height: "229px", width: "344px" }}
          src="https://cutrencay.com/wp-content/uploads/2020/08/khu-sinh-hoat-chung-e1632355518787-600x398.jpg"
          alt=""
        />
        <img
          style={{ height: "229px", width: "344px" }}
          src="https://cutrencay.com/wp-content/uploads/2020/08/SHC-1-e1632355419683-599x400.jpg"
          alt=""
        />
        <img
          style={{ height: "229px", width: "344px" }}
          src="https://cutrencay.com/wp-content/uploads/2020/08/BEPCU-e1632358089144-600x399.jpg"
          alt=""
        />
      </p>
      <p style={{ textAlign: "justify" }}>
        Ngoài ra, Cú khách có thể sử dụng sân khu sinh hoạt để ăn uống, nướng
        BBQ, nhưng chú ý tránh gây ồn ào làm ảnh hưởng đến các Cú khách khác.
      </p>
      <p>&nbsp;</p>
      <p style={{ textAlign: "center" }}>
        <img
          style={{height: "229px", width: "344px" }}
          src="https://cutrencay.com/wp-content/uploads/2020/08/bepchung-scaled-e1632488566429-600x400.jpg"
          alt=""
        />
        <img
          style={{ height: "229px", width: "344px" }}
          src="https://cutrencay.com/wp-content/uploads/2020/08/110050680_2982464388546225_47786574114152612_n-1-e1632356111747-600x400.jpg"
          alt=""
        />
        <img
          style={{ height: "229px", width: "344px" }}
          src="https://cutrencay.com/wp-content/uploads/2020/08/BEP3-1-scaled-e1632357673693-599x400.jpeg"
          alt=""
        />
      </p>
      <p style={{ textAlign: "justify" }}>
        Bếp chung rộng rãi, có m
        <span style={{ fontSize: "16px" }}>
          ì gói, trà, cà phê, trứng miễn phí cho bữa sáng, đầy đủ đồ dùng và gia
          vị cần thiết,{" "}
        </span>
        Cú khách tự phục vụ và dọn dẹp sau khi dùng.
      </p>
      <p>&nbsp;</p>
      <p style={{ textAlign: "justify" }}>
        <span style={{ fontSize: "16px" }}>
          <strong>
            Để biết thêm thông tin chi tiết và đặt phòng nhanh chóng bạn vui
            lòng liên hệ HOTLINE{" "}
            <span style={{ color: "#ed1c24" }}>0123.456.789 </span> nha !&nbsp;
          </strong>
        </span>
      </p>
    </div>
  );
}
